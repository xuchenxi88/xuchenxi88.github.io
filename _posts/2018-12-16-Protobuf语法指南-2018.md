...
## 目录 [−]
 1. [定义一个消息类型](#定义一个消息类型)
	...
	- [1.2 分配标识号](#2-分配标识号)
	- [1.3 指定字段规则](#3-指定字段规则)
	- [1.4 添加更多消息类型](#4-添加更多消息类型)
	- [1.5 添加注释](#5-添加注释)
	- [1.6 从.proto文件生成了什么？](#6-从proto文件生成了什么？)
	- [1.7 标量数值类型](#7-标量数值类型)
	- [1.8 Optional的字段和默认值](#8-optional的字段和默认值)
	- [1.9 枚举](#9-枚举)
 2. [使用其他消息类型](#使用其他消息类型)
	- [2.1 导入定义](#1-导入定义)
	- [2.2 嵌套类型](#2-嵌套类型)
	- [2.3 组](#3-组)
	- [2.4 更新一个消息类型](#4-更新一个消息类型)
	- [2.5 扩展](#5-扩展)
	- [2.6 嵌套的扩展](#6-嵌套的扩展)
	- [2.7 选择可扩展的标量符号](#7-选择可扩展的标量符号)
 3. [Oneof](#oneof)
	 - [3.1 使用Oneof](#1-使用oneof)
	 - [3.2 向后兼容性问题](#2-向后兼容性问题)
 4. [包Package](#包package)
	- [4.1. 包及名称的解析](#1-包及名称的解析)
 5. [定义服务Service](#定义服务(service))
 6. [选项Options](#选项options)
	- [6.1 自定义选项](#1-自定义选项)
 7. [生成访问类](#生成访问类)
